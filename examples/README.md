...
To gang-schedule a group of tasks all-or-nothing, add them to an `armada_flyte.Gang` and `await` it;
the gang id and cardinality are derived from the members, so there is nothing to keep in sync:

```python
from armada_flyte import Gang

# one worker per item, all co-scheduled or none (see gang.py). dag.py shows the add()/run() form
parts = await Gang.map(worker, range(4), node_uniformity_label="kubernetes.io/hostname")
```
